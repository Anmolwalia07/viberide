        ...
        <div className="mt-16 grid gap-6 md:grid-cols-3">
          {areas.map((area) => (
            <Link
              ...
              className="card group block p-7 transition-transform duration-300 hover:-translate-y-1"
            >
              <div className="eyebrow">Service area</div>
              <h2 className="serif mt-5 text-3xl text-white transition-colors group-hover:text-[#d2bd8e]">{area.name}</h2>
              <p className="mt-3 text-sm leading-6 text-neutral-400">{area.description}</p>
              <span className="mt-6 inline-flex items-center gap-2 text-sm font-medium text-[#d2bd8e]">
                Explore {area.name}
              </span>
            </Link>
          ))}
